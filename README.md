...
- [x] **Pérdida de comunicación con comportamiento definido, en ambos sentidos (24 sept 2026).** Además del watchdog Python (14 sept, CODESYS→Python), se agregó `heartbeat` (registro 12) y `comm_error` del lado CODESYS (Python→CODESYS): si el servidor Python se cae, la alarma se enciende y `heater_cmd`/`fan_cmd` se fuerzan a 0 de verdad — verificado con CODESYS real. Detalle, código ST y evidencia en `codesys/comunicacion_modbus.md`, sección 2.3.
- [x] `SecadoCafe_ModbusV1.project` actualizado ya está en el repositorio — ver nota de ubicación en `codesys/README.md` (quedó en `codesys/archivo_codesys/`, no directo en `codesys/`; pendiente decidir si se mueve o se deja así).
