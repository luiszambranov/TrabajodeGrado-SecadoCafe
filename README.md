...
- [x] Prueba mínima de comunicación. Interfaz elegida: **Modbus TCP** (Python = servidor/slave, CODESYS = cliente/master; ver `codesys/comunicacion_modbus.md`). **Verificada end-to-end el 9-10 sept 2026, con HMI**: `T_process` y `heater_cmd` confirmados en ambos sentidos entre `python/modbus_server.py` y `codesys/ModbusPhytoon.project` (CODESYS V3.5 SP15), y mostrados en vivo en un HMI mínimo dentro de CODESYS (valor numérico de `T_process` + piloto del calentador). Evidencia en `codesys/evidencia_hmi_modbus.png` (HMI y consola de Python con el mismo valor en el mismo instante). Pendiente: comportamiento ante pérdida de comunicación. Hay además una prueba de conexión CODESYS-FluidSIM, `codesys/ConcexionOPC.png`, pendiente de formalizar por separado.
- [x] HMI en CODESYS: valor numérico de `T_process` en vivo + piloto del calentador (`codesys/comunicacion_modbus.md`, sección 5). Pendiente para subir complejidad: más "pilotos"/variables simuladas en Python (`fan_cmd`, `RH_ambient`), tendencia (Trend) y alarma de pérdida de comunicación — plan detallado en `codesys/comunicacion_modbus.md`, sección 6.
